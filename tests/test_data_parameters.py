...
class TestDataParametersMarkdown(unittest.TestCase):
    ...
    @patch(
        "melektrodica.Tool.unit_conversion", return_value=296.15
    )  # Conversión 23°C -> Kelvin
    ...
    def test_initialization(self, mock_raw_data, mock_unit_conversion):
        ...
        self.assertAlmostEqual(
            params.temperature, 296.15, places=2
        )  # Temperatura en Kelvin
        self.assertEqual(
            params.potential.tolist(),
            list(
                np.arange(0.0, 0.5 + 0.001, 0.001)
            ),  # Potenciales esperados [0.0, 0.001, ..., 0.5]
        )
        ...
        self.writer.message.assert_any_call(
            f"Reading parameters from file: {self.test_file_path}"
        )
        self.writer.message.assert_any_call(
            "Thermochemical part details:\n\tDG_reaction: False\n\tG_formation: True"
        )
